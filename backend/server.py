    ...
    testimonials = [
        {
            "id": str(uuid.uuid4()),
            "name": "Алексей Петров",
            "role": "Владелец кафе 'Арт-Пространство'",
            "text": "Ребята из 'Контраст' превратили наше кафе в настоящее произведение искусства! Теперь у нас всегда полно посетителей, которые приходят не только за кофе, но и за эстетическим удовольствием.",
            "rating": 5,
            "approved": True,
            "created_at": datetime.utcnow()
        },
        {
            "id": str(uuid.uuid4()),
            "name": "Мария Сидорова",
            "role": "Директор торгового центра",
            "text": "Заказывали художественное оформление детской зоны. Работа выполнена быстро, качественно и с полным соблюдением сроков. Дети в восторге от ярких персонажей!",
            "rating": 5,
            "approved": True,
            "created_at": datetime.utcnow()
        },
        {
            ...
            "text": "Граффити на стене автосервиса стало настоящей достопримечательностью района. Качество работы превзошло все ожидания. Рекомендую всем!",
            "rating": 5,
            "approved": True,
            "created_at": datetime.utcnow()
        }
    ...
    faqs = [
        {
            ...
            "answer": "Время выполнения зависит от сложности и масштаба проекта. Простые работы до 20 м² выполняем за 1-2 дня, крупные проекты могут занимать от недели до месяца. Точные сроки обсуждаются индивидуально.",
            "order": 1,
            "active": True,
            "created_at": datetime.utcnow()
        },
        {
            ...
            "answer": "Мы предоставляем гарантию до 5 лет на все виды художественных работ при соблюдении условий эксплуатации. Гарантия покрывает сохранность цвета и отсутствие отслоения краски.",
            "order": 2,
            "active": True,
            "created_at": datetime.utcnow()
        },
        {
            ...
            "answer": "Да, мы осуществляем выезды и берём заказы по всей территории России. Для проектов в других регионах рассчитываются транспортные расходы и проживание специалистов.",
            "order": 3,
            "active": True,
            "created_at": datetime.utcnow()
        },
        {
            ...
            "answer": "Стоимость рассчитывается индивидуально исходя из площади, сложности дизайна, типа поверхности и используемых материалов. Воспользуйтесь нашим калькулятором для предварительной оценки.",
            "order": 4,
            "active": True,
            "created_at": datetime.utcnow()
        },
        {
            "id": str(uuid.uuid4()),
            "question": "Можно ли реализовать собственный дизайн?",
            "answer": "Конечно! Мы работаем как с собственными эскизами, так и воплощаем ваши идеи. Также можем доработать ваш концепт с учётом технических особенностей поверхности.",
            "order": 5,
            "active": True,
            "created_at": datetime.utcnow()
        }
